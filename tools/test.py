...
def parse_args():
    ...
    parser.add_argument("config",type=str, default='./configs/DUN_RNNViT/dun_rnnvit.py')
    parser.add_argument("--work_dir",type=str, default='./work_dirs')
    parser.add_argument("--weights",type=str, default='./checkpoints/dadun_5i64ch_gray.pth')
        ...
        for ii in range(batch_size):
            ...
            with torch.no_grad():
                ...
                if "amp" in cfg.keys() and cfg.amp:
                    ...
                    #torch.Size([1, 1, 256, 256]),torch.Size([1, 8, 256, 256]),torch.Size([1, 1, 256, 256])
                    outputs = model(single_meas, Phi, Phi_s) #torch.Size([1, 8, 256, 256])
                torch.cuda.synchronize()
                end = time.time()
                run_time = end - start
                if ii>0:
